...
def compute_intermediate(y, z):
    """Compute x' = y + z"""
    return y + z


def compute_final(x, y_plus_z):
    """Compute x'' = x + x'"""
    return x + y_plus_z
...
variables = ["template", "x", "y", "z", "y_plus_z", "result", "raw_input", "raw_output"]
...
values = {
    ...
    "y": range(1,NUM_RANGE),
    "z": range(1,NUM_RANGE),
    "y_plus_z": range(2, NUM_RANGE * 2 - 1),  # y + z can be up to 198
...
def intervene_on_intermediate():
    """
    Generate a counterfactual where we intervene on the intermediate variable y_plus_z.
    ...
    # Change y_plus_z to a different value
    current_intermediate = addition_causal_model.run_forward(input_sample)["y_plus_z"]
    possible_values = [i for i in range(2, NUM_RANGE * 2 - 1) if i != current_intermediate]
    counterfactual["y_plus_z"] = random.choice(possible_values)
...
def distinct_counterfactual():
    ...
    while True:
        ...
        if x0 != x1 and y0 != y1 and z0 != z1 and y0+z0 != y1+z1 and x0+y0+z0 != x1+y1+z1:
            break
    return {"input": input_sample, "counterfactual_inputs": [counterfactual]}
