...
values = {
    ...
    "x": range(1, NUM_RANGE),
    "y": range(1, NUM_RANGE),
    "result": range(1, NUM_RANGE * NUM_RANGE),  # x * y can be up to 361
    "raw_input": None,
    "raw_output": None,
}
...
multiplication_causal_model = CausalModel(
    variables,
    values,
    parents,
    mechanisms,
    id="two_number_multiplication"
)


# Dataset Sampler Functions
def sample_multiplication_problem():
    """Sample a random multiplication problem."""
    input_sample = multiplication_causal_model.sample_input()
    input_sample["raw_input"] = multiplication_causal_model.run_forward(input_sample)["raw_input"]
    return input_sample
...
def intervene_on_x():
    ...
    """
    input_sample = sample_multiplication_problem()
    ...
    input_sample["raw_input"] = multiplication_causal_model.run_forward(input_sample)["raw_input"]
    counterfactual["raw_input"] = multiplication_causal_model.run_forward(counterfactual)["raw_input"]
...
def random_counterfactual():
    ...
    """
    input_sample = sample_multiplication_problem()
    counterfactual = sample_multiplication_problem()
    return {"input": input_sample, "counterfactual_inputs": [counterfactual]}
...
def get_factor_index(input_sample, pipeline, position):
    """
    Find the index of a factor (x or y) in the prompt.
    ...
    Returns:
        list[int]: List containing the index of the factor token (last token of the number)
    """
...
def create_factor_token_position(pipeline, position):
    """
    Create a TokenPosition for a factor (x or y).
    ...
    Returns:
        TokenPosition: Token position for the specified factor
    """
    factor_names = ["x", "y"]
    return TokenPosition(
        lambda x: get_factor_index(x, pipeline, position),
        pipeline,
        id=factor_names[position]
    )
...
multiplication_task = Task(
    name="multiplication",
    causal_models={
        "two_number_multiplication": multiplication_causal_model,
    },
    dataset_generators={
    ...
    token_positions={
        "x": lambda x: create_factor_token_position(x, 0),
        "y": lambda x: create_factor_token_position(x, 1),
